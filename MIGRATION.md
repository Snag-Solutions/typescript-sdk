...
This guide outlines the changes and steps needed to migrate your codebase to the latest version of the Snag Solutions TypeScript SDK.
...
To use it, upgrade the `@snagsolutions/sdk` package, then run `./node_modules/.bin/snagsolutions-sdk migrate ./your/src/folders` to update your code.
...
The `httpAgent` client option has been removed in favor of a [platform-specific `fetchOptions` property](https://github.com/stainless-sdks/snag-solutions-typescript#fetch-options).
This change was made as `httpAgent` relied on `node:http` agents which are not supported by any runtime's builtin fetch implementation.

If you were using `httpAgent` for proxy support, check out the [new proxy documentation](https://github.com/stainless-sdks/snag-solutions-typescript#configuring-proxies).
...
#### Refactor of `@snagsolutions/sdk/core`, `error`, `pagination`, `resource` and `uploads`

Much of the `@snagsolutions/sdk/core` file was intended to be internal-only but it was publicly accessible, as such it has been refactored and split up into internal and public files, with public-facing code moved to a new `core` folder and internal code moving to the private `internal` folder.
...
```typescript
// Before
import '@snagsolutions/sdk/error';
import '@snagsolutions/sdk/pagination';
import '@snagsolutions/sdk/resource';
import '@snagsolutions/sdk/uploads';

// After
import '@snagsolutions/sdk/core/error';
import '@snagsolutions/sdk/core/pagination';
import '@snagsolutions/sdk/core/resource';
import '@snagsolutions/sdk/core/uploads';
```

If you were relying on anything that was only exported from `@snagsolutions/sdk/core` and is also not accessible anywhere else, please open an issue and we'll consider adding it to the public API.

#### Resource classes

Previously under certain circumstances it was possible to import resource classes like `Assets` directly from the root of the package. This was never valid at the type level and only worked in CommonJS files.
Now you must always either reference them as static class properties or import them directly from the files in which they are defined.

```typescript
// Before
const { Assets } = require('@snagsolutions/sdk');

// After
const { SnagSolutions } = require('@snagsolutions/sdk');
SnagSolutions.Assets; // or import directly from @snagsolutions/sdk/resources/assets
```

#### Cleaned up `uploads` exports

As part of the `core` refactor, `@snagsolutions/sdk/uploads` was moved to `@snagsolutions/sdk/core/uploads`
...
```typescript
import { type Uploadable, toFile } from '@snagsolutions/sdk/core/uploads';
```
...
```typescript
// Before
import { APIClient } from '@snagsolutions/sdk/core';
...
```ts
// Before
SnagSolutions.fileFromPath('path/to/file');
...
The `@snagsolutions/sdk/shims` imports have been removed. Your global types must now be [correctly configured](#minimum-types-requirements).

### `@snagsolutions/sdk/src` directory removed

Previously IDEs may have auto-completed imports from the `@snagsolutions/sdk/src` directory, however this
directory was only included for an improved go-to-definition experience and should not have been used at runtime.

If you have any `@snagsolutions/sdk/src/*` imports, you will need to replace them with `@snagsolutions/sdk/*`.

```ts
// Before
import SnagSolutions from '@snagsolutions/sdk/src';
