...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['SNAG_SOLUTIONS_LOG'] or 'warn' if it isn't set.
   */
...
export class SnagSolutions {
   ...
   */
  constructor({
    baseURL = readEnv('SNAG_SOLUTIONS_BASE_URL'),
    apiKey = readEnv('X_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
    ...
    this.timeout = options.timeout ?? SnagSolutions.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SNAG_SOLUTIONS_LOG'), "process.env['SNAG_SOLUTIONS_LOG']", this) ??
      defaultLogLevel;
