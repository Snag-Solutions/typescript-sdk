...
import { APIPromise } from '@snagsolutions/sdk/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['SNAG_SOLUTIONS_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: SnagSolutions) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['SNAG_SOLUTIONS_LOG'] = 'debug';
      const client = new SnagSolutions({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['SNAG_SOLUTIONS_LOG'] = 'not a log level';
      const client = new SnagSolutions({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'SNAG_SOLUTIONS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
